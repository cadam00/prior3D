...
## **Introduction to the prior3D Package (tutorial)**

The **prior3D** package offers a comprehensive toolset for 3D systematic conservation planning, conducting nested prioritization analyses across multiple depth levels and ensuring efficient resource allocation throughout the water column [@doxa20224d]. It provides a structured workflow designed to address biodiversity conservation and management challenges in the 3 dimensions, such as the incorporation of multiple costs at different depth levels, while facilitating users’ choices and parameterization. The process initiates from the deepest level and progressively moves toward the surface, by conducting a step-by-step prioritization analysis at each depth Figure \@ref(fig:Figure1). The optimization result at each depth level is considered as a cost layer for the layer above. This approach gives priority to areas chosen in deeper waters when selecting areas at the subsequent upper level, thus creating a cascading prioritization structure. The **prior3D** package is built upon the **prioritizr** package [@prioritizr], using commercial and open-source exact algorithm solvers that ensure optimal solutions to prioritization problems.

```{r Figure1, eval=TRUE, echo=FALSE, fig.cap="Flow chart of the 3D prioritization analysis for the four depth zones considered in the @doxa20224d analysis", out.width = '100%', fig.align = "center"}
...
a.  Use the `split_rast()` function to convert 2D distribution rasters of biodiversity features into a 3D format.

b.  Use the `prioritize_3D()` function to set the optimization problem and define its parameters. This function also solves the problem and provides the solution in the form of a map.

c.  Use the `evaluate_3D()` function to obtain detailed results in a tabular format.

d.  Use the `plot_3D()` function to generate graphs based on the solution results.
...
Let us consider the following dataset as an illustrative example. It represents a subset of the species analyzed in @doxa20224d. For simplicity reasons, we have included only 20 species for demonstration purposes.
...
### **Step 2: 3D Prioritization Algorithm**
...
```{r Figure3, eval=TRUE, echo=FALSE, out.width = '100%', fig.cap="Prioritization maps for single and multiple budget percentages", fig.align = "center"}
...
```{r Figure4, eval=TRUE, echo=FALSE, fig.cap="Output for 30% budget percentage", out.width = '60%', fig.align = "center"}
...
```{r Figure5, eval=TRUE, echo=FALSE, fig.cap="Output for multiple budget percentages", out.width = '75%', fig.align = "center"}
knitr::include_graphics("fig5.jpg")
```

## **Comparative Analysis of a 2D and a 3D SCP approach**
...
```{r Figure6, eval=TRUE, echo=FALSE, fig.cap="Comparison of 2D vs 3D approach for multiple budget percentages", out.width = '75%', fig.align = "center"}
...
High SA values signify that there is a high spatial heterogeneity, indicating lower spatial coherence. High SKU indicate high spatial coherence. Both SA and SKU are calculated using R package **geodiv** [@geodiv], applying `geodiv::focal_metrics` functions "sa" and "sku" to optimization solution rasters.

High RAO values suggest increased spatial heterogeneity, thus low spatial coherence. To compute the RAO metric, a moving window approach is employed on optimization solution maps, using function `rasterdiv::paRao` from R package **rasterdiv** [@rasterdiv; @rasterdivpaper]. The dimensions of the window chosen is 3×3. The new raster, which is a result of the application of the algorithm, is used to get an average RAO value for the whole raster.
...
```{r Figure7, eval=TRUE, echo=FALSE, fig.cap="SA", out.width = '75%', fig.align = "center"}
...
```{r Figure8, eval=TRUE, echo=FALSE, fig.cap="SKU", out.width = '75%', fig.align = "center"}
...
```{r Figure9, eval=TRUE, echo=FALSE, fig.cap="RAO", out.width = '75%', fig.align = "center"}
